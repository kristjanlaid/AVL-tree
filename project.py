...
class AVLTree(object):
    ...
    def parse_and_execute_command(command, tree):
        parts = command.lower().replace('.', '').replace(',', '').split()
        if not tree:
            tree = AVLTree()
            
        if 'create' in parts and 'tree' in parts:
            if 'nodes' in parts:
                nodes_index = parts.index('nodes')
                num_nodes = int(parts[nodes_index - 1])

            if 'values' in parts and 'from' in parts and 'to' in parts:
                from_index = parts.index('from') + 1
                to_index = parts.index('to') + 1

                if from_index < len(parts) and to_index < len(parts):
                    from_value = int(parts[from_index])
                    to_value = int(parts[to_index])

                    for i in range(num_nodes):
                        value = from_value + i * (to_value - from_value) // (num_nodes - 1)
                        tree.insert(value)

            
        # Example cases to add/remove
        # parse_and_execute_command("Add 46", avl_tree)
        # parse_and_execute_command("Remove 52", avl_tree)


        elif 'add' in parts:

            value_index = parts.index('add') + 1

            if value_index < len(parts):

                value = int(parts[value_index])

                print(f"Adding value {value} to the tree...")

                # insert method here

        elif 'remove' in parts:

            value_index = parts.index('remove') + 1

            if value_index < len(parts):

                value = int(parts[value_index])

                print(f"Removing value {value} from the tree...")
